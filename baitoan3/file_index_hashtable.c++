...
// find min value in each first elemmet of hash table
int find_min(HashTable HT) {
    string min_string = "zzzzzzzzzzzz";
    int index = -1;
    for (int i = 0; i < N; i++) {
        if (HT[i]!=NULL) {
            if (compare(HT[i]->value, min_string)) {
                min_string = HT[i]->value;
                index = i;
            }
        }
    }
...
indexNode *checkExist(string word, HashTable &HT) {
    int index = Hash(word);
    indexNode *p = HT[index];
    while (p != NULL) {
        if (checkBalance(word,p->value )) {
            break;
        }
        p = p->next;
    }
    return p; // return node where this word exists
}

// read file vanban.txt
void readMainFlie(HashTable &HT, Node *head) {
            ...
            if ((int(x)<97 && int(x) > 90) || int(x) > 122|| int(x) <65 ||x == '\n') {
                if (word.length()!=0) {
                    if (check(word, head) || word == "s") {
                        ...
                        // if this word is meaning
                        if (checkExist(word, HT)!=NULL) {
                            checkExist(word, HT)->count++;
                            if (checkExist(word, HT)->end->value != index) {
                                NodeCount *ref = new NodeCount();
                                ref = checkExist(word, HT)->begin;
                                while (ref->next != NULL) {
                                    ref = ref->next;
                                }
                                ...
                                n->next = NULL;
                                ref->next = n;
                                checkExist(word, HT)->end->value = index;
                            } 
                        }
                        else {
                            indexNode *headIndexRef = new indexNode();
                            headIndexRef = HT[Hash(word)];
                            ...
                            if (HT[Hash(word)] == NULL) {
                                HT[Hash(word)] = n;
                            }
                            else {
                                if (!compare((HT[Hash(word)])->value,n->value)) {
                                    n->next = HT[Hash(word)];
                                    HT[Hash(word)] = n;
                                }
                                else {
...
int main() {
    ...
    readMainFlie(hashTable, stopWord);
